...
coverage/classification and capability JSON, registration/unregistration,
analysis and preview, stale-result refusal, safe assignment, every documented
material-identity transition, preservation, save/reopen, FBX export/reimport,
and anonymized synthetic material characterization.
...
- [x] Verify alpha, significance, address, image override, channel override,
  and UV override controls.
...
- [x] Press Escape and confirm cancellation leaves no partial report or data
  change.
- [x] Verify class counts and the suppressed/unsupported reporting locations.
...
- [x] Review planned materials, reuse, skips, faces to reassign, and estimated
  slot/section increase before assignment.
- [x] Assign directly from the Edit Mode preview and verify the intended material
  partition.
- [x] Undo and redo assignment from the 3D View.
- [x] Rerun and confirm `Faces to reassign: 0` and `Additional slots: 0`; the
  idempotence regression tests also assert no duplicate datablocks or slots.
